# Embedded materials, authoritative Canvas

SystemStudio uses one preparation sequence for every lecture: **Accessible HTML lecture → Read → Watch → Practice 8 questions → Build/debug where mapped**. The responsive HTML lecture is the primary format and supplies direct novice-facing explanations, examples, self-checks, and source-bounded tutor prompts; the untagged PDF is only an optional visual archive. Five distinct attempts complete the preparation checkpoint; all eight provide broader practice. These counts do not certify readiness or mastery. Each answer includes an explanation, justification, and mapped evidence from focused open-book sections, transcript-checked author videos, and legacy slide evidence. Those authoritative sources open externally; the extension does not copy the book or use Google Drive.

SystemStudio includes these resources inside the extension:

- 13 primary accessible HTML lectures and 13 optional, integrity-checked visual PDF archives;
- Homework 1, Homework 2, and Homework 3 references; and
- three processor-project milestones that build one cumulative 4-bit processor and a final-presentation planning reference.

The Learning Center maps focused open-book sections, transcript-checked author videos, and lecture-slide evidence to all 13 presentation resources, then adds 104 formative questions—exactly eight for every module. It supports immediate-feedback practice, end-of-session quiz review, confidence checks, saved questions, and local spaced review.

The presentations are packaged locally, so no external document-hosting account is needed. The Fall 2026 Canvas course controls deadlines, requirements, and submission.
