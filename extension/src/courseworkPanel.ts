...
export class CourseworkPanel implements vscode.Disposable {
    ...
    try {
      switch (value.type) {
        ...
        case 'run-preflight':
          ...
          return;
        case 'open-design-tutor':
          if (isCourseworkId(value.id)) {
            await this.openDesignTutor(value.id, typeof value.contractId === 'string' ? value.contractId : undefined, value.failed === true);
          }
          return;
        case 'open-guided-assembly':
          if (value.id === 'homework-03') {
            await vscode.commands.executeCommand('systemstudioCis310.openGuidedLabs', 'assembly-linear-search');
          } else if (value.id === 'final-project') {
            await vscode.commands.executeCommand('systemstudioCis310.openGuidedLabs', 'assembly-binary-search-iterative');
          }
          return;
        case 'export-bundle':
  ...
  private async openDesignTutor(id: CourseworkId, requestedContractId?: string, failed = false): Promise<void> {
    const contracts = circuitPreflightsForCoursework(id);
    let contract = requestedContractId ? circuitPreflightById(requestedContractId) : undefined;
    if (contract && !contract.courseworkIds.includes(id)) contract = undefined;
    if (!contract) {
      const choice = await vscode.window.showQuickPick(
        contracts.map((candidate) => ({
          label: candidate.label,
          description: 'attempt-first design coaching',
    ...
    await vscode.commands.executeCommand('systemstudioCis310.openAiTutor', {
      circuitPreflightId: contract.id,
      tutorMode: failed ? 'failed-preflight' : 'design'
    });
  }
...
function courseworkHtml(webview: Pick<vscode.Webview, 'cspSource'>, state: object): string {
...
function courseCard(item,compact=false){const p=itemState(item.id),title=esc(item.title);const checks=item.checks.map(check=>'<label class="check"><input type="checkbox" data-check="'+esc(check.id)+'" data-id="'+esc(item.id)+'" '+(p.completedCheckIds.includes(check.id)?'checked':'')+'><span>'+esc(check.label)+'</span></label>').join('');const guidedAssembly=item.id==='homework-03'||item.id==='final-project'?'<button class="secondary" data-guided-assembly="'+esc(item.id)+'" aria-label="Start the guided assembly search walkthrough for '+title+'">Start assembly search walkthrough</button>':'';const toolActions=(item.kind==='implementation'||item.kind==='final'?'<button class="secondary" data-preflight="'+esc(item.id)+'" aria-label="Run local circuit preflight for '+title+'">Run local circuit preflight</button><button class="secondary" data-design-tutor="'+esc(item.id)+'" aria-label="Ask the AI tutor for a design hint for '+title+'">Ask tutor for a design hint</button>':'')+(item.expectedExtensions.includes('asm')?guidedAssembly+'<button class="secondary" data-command="systemstudioCis310.buildRunAssembly" aria-label="Build or run assembly for '+title+'">Build/run assembly</button>':'');return '<article class="course-card '+(item.kind==='final'?'final':'')+'"><header><div class="stage">'+esc(item.stage)+'</div><h3>'+title+'</h3><p>'+esc(item.summary)+'</p></header><div class="body"><p><strong>Prepare:</strong> '+item.preparation.map(esc).join(' · ')+'</p><div class="status-row"><label>Status <select data-status="'+esc(item.id)+'" aria-label="Local status for '+title+'">'+['not-started','in-progress','ready-to-submit','submitted','receipt-confirmed'].map(s=>'<option value="'+s+'" '+(p.status===s?'selected':'')+'>'+statusLabel(s)+'</option>').join('')+'</select></label></div><div class="checks">'+checks+'</div><div class="actions"><button data-resource="'+esc(item.id)+'" aria-label="Open planning reference for '+title+'">Open planning reference</button>'+toolActions+'<button class="secondary" data-validate="'+esc(item.id)+'" aria-label="Check selected files for '+title+'">Check selected files</button><button class="secondary" data-bundle="'+esc(item.id)+'" aria-label="Create planning ZIP for '+title+'">Create planning ZIP</button><button class="quiet" data-command="systemstudioCis310.openCanvas" aria-label="Open Canvas for '+title+'">Open Canvas</button></div><div class="validation" role="status" aria-live="polite" data-validation-id="'+esc(item.id)+'"></div></div></article>'}
