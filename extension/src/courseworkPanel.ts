...
export class CourseworkPanel implements vscode.Disposable {
  ...
  static show(context: vscode.ExtensionContext, store: PracticeStore, materials: CourseMaterials): void {
    ...
    CourseworkPanel.current = new CourseworkPanel(context, store, materials);
  }
  ...
  private constructor(
    ...
    this.panel = vscode.window.createWebviewPanel(
      ...
      'CIS 310 Coursework and Final Project',
...
async function inspectSelectedFiles(item: typeof COURSEWORK_CATALOG[number], selected: readonly vscode.Uri[]): Promise<{ lines: string[]; warnings: string[] }> {
  ...
  if (item.kind === 'final') {
    if (!extensions.has('dig')) warnings.push('No 8-bit processor .dig circuit was selected.');
...
<section class="panel" role="tabpanel" id="capstone" aria-labelledby="tab-capstone" hidden><h2>Final-project progression and self-evaluation</h2><div class="callout"><strong>Width upgrade:</strong> Project 3 is the packaged integrated 4-bit milestone. The separate final project culminates in an 8-bit processor and assembly-program demonstration. Canvas will clarify the required ISA/toolchain and exact artifacts.</div><div id="final-card"></div><section class="card"><h3>How ready do I feel today?</h3><p class="muted">Rate evidence, not optimism: 1 = I need guided help; 3 = I can do it with references; 5 = I can demonstrate and explain it independently.</p><div id="self-ratings" class="ratings"></div><p id="self-summary" class="muted"></p></section></section>
<section class="panel" role="tabpanel" id="grade" aria-labelledby="tab-grade" hidden><h2>Manual final-grade planning estimate</h2><div class="callout"><strong>Not an official grade.</strong> Copy scores from Canvas manually. This local calculator applies the published 15% participation-quiz, 65% homework/implementation, and 20% final-project weights. It identifies the two lowest participation-item percentages, drops those rows, and combines the retained earned/possible points. Canvas and instructor/GSI decisions remain authoritative.</div><form id="grade-form"><fieldset><legend>Participation quizzes and in-class evidence checks · 15%</legend><p class="muted">Enter at least three items. The two lowest percentages will be dropped; retained points are then combined so unequal point values remain unequal.</p><div id="quiz-scores"></div><button type="button" class="secondary" id="add-quiz">Add participation item</button></fieldset><fieldset><legend>Three homework + three implementation assignments · 65%</legend><label>Current or anticipated Canvas category percentage <input id="coursework-percent" type="number" min="0" max="100" step="0.01" required></label><p class="muted">Use the category percentage shown/calculated under the current Canvas rules; this avoids guessing how different point totals are normalized.</p></fieldset><fieldset><legend>Final 8-bit processor project and demonstration · 20%</legend><div class="score-grid"><span>Final project</span><label><span class="sr-only">earned</span><input id="final-earned" type="number" min="0" step="0.01" placeholder="earned" value="" aria-label="Final project earned points" required></label><label><span class="sr-only">possible</span><input id="final-possible" type="number" min="0.01" step="0.01" placeholder="possible" value="" aria-label="Final project possible points" required></label><span></span></div></fieldset><button type="submit">Calculate planning estimate</button></form><div id="grade-result" aria-live="polite"></div></section>
