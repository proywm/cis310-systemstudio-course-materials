...
export class StudentHelperPanel implements vscode.Disposable {
  ...
  private constructor(context: vscode.ExtensionContext) {
    ...
    const companion = this.panel.webview.asWebviewUri(
      vscode.Uri.joinPath(context.extensionUri, 'media', 'tutor-companion.png')
    );
    this.panel.webview.html = helperHtml(this.panel.webview, companion);
...
function helperHtml(webview: vscode.Webview, companion: vscode.Uri): string {
...
form{display:flex;gap:8px;position:sticky;bottom:0;padding:14px 0;background:var(--vscode-editor-background)}input{flex:1;padding:9px 10px;color:var(--vscode-input-foreground);background:var(--vscode-input-background);border:1px solid var(--vscode-input-border)}form button,.support-card button{border:0;padding:8px 15px;color:var(--vscode-button-foreground);background:var(--vscode-button-background);cursor:pointer}.chat-widget{position:fixed;right:22px;bottom:20px;z-index:4;display:grid;justify-items:end;gap:10px}.welcome{width:min(380px,calc(100vw - 44px));border:1px solid var(--vscode-panel-border);border-radius:12px;padding:14px;background:var(--vscode-editorWidget-background);box-shadow:0 8px 28px var(--vscode-widget-shadow)}.welcome[hidden]{display:none}.welcome-head{display:grid;grid-template-columns:72px minmax(0,1fr) auto;gap:12px;align-items:center}.welcome h2{margin:0 0 5px;font-size:1.08rem}.welcome p{margin:0;line-height:1.45}.welcome-close{align-self:start;border:0;background:transparent;color:var(--vscode-foreground);cursor:pointer;font-size:1.2rem}.companion{width:72px;height:72px;object-fit:contain;filter:drop-shadow(0 4px 6px rgba(0,0,0,.28));transform-origin:50% 90%;animation:companion-breathe 2.4s ease-in-out infinite}.chat-bubble{width:66px;height:66px;border:0;border-radius:50%;display:grid;place-items:center;overflow:hidden;background:var(--vscode-button-background);box-shadow:0 6px 20px var(--vscode-widget-shadow);cursor:pointer}.chat-bubble img{width:64px;height:64px;object-fit:cover;object-position:center 20%;animation:companion-wave 2.8s ease-in-out infinite}.chat-bubble:focus-visible{outline:3px solid var(--vscode-focusBorder);outline-offset:3px}.motion-control{margin:10px 0 0 84px;border:1px solid var(--vscode-panel-border);border-radius:4px;padding:4px 8px;color:var(--vscode-foreground);background:transparent;cursor:pointer;font-size:.78rem}.motion-paused .companion,.motion-paused .chat-bubble img{animation:none}@keyframes companion-breathe{0%,100%{transform:translateY(0) rotate(-1deg)}50%{transform:translateY(-5px) rotate(1deg)}}@keyframes companion-wave{0%,45%,100%{transform:translateY(0) rotate(0)}55%{transform:translateY(-2px) rotate(-3deg)}65%{transform:translateY(-3px) rotate(3deg)}75%{transform:translateY(-2px) rotate(-2deg)}}@media(prefers-reduced-motion:reduce){.companion,.chat-bubble img{animation:none!important}.motion-control{display:none}}@media(max-width:650px){.support-grid{grid-template-columns:1fr}.welcome-head{grid-template-columns:56px minmax(0,1fr) auto}.companion{width:56px;height:56px}.motion-control{margin-left:68px}}
