...
content.querySelectorAll('[data-source]').forEach(button=>button.onclick=()=>vscode.postMessage({type:'open-source',labId:lab.id,source:button.dataset.source}));content.querySelectorAll('[data-step]').forEach(input=>input.onchange=()=>vscode.postMessage({type:'toggle-step',labId:lab.id,stepId:input.dataset.step,completed:input.checked}));document.getElementById('artifact').onclick=()=>vscode.postMessage({type:'open-artifact',labId:lab.id});if(lab.kind==='assembly')document.getElementById('assembly-tutor').onclick=()=>vscode.postMessage({type:'open-tutor',labId:lab.id});document.getElementById('reset').onclick=()=>{if(confirm('Reset the self-reported checkmarks for this lab?'))vscode.postMessage({type:'reset-lab',labId:lab.id})}}
