...
const vscode=acquireVsCodeApi();const TOPICS=${topics},GUIDED_LABS=${guidedLabs};let dashboard=${initialDashboard};let learningPath=${initialLearningPath};let currentQuestion=null;let selectedIndex=null;let confidence=null;let usedHint=false;let startedAt=Date.now();let answered=false;
...
function labButtons(resourceId){return labsFor(resourceId).map(lab=>'<button class="secondary" data-guided-lab="'+esc(lab.id)+'">'+(lab.kind==='circuit'?'Build: ':'Trace: ')+esc(lab.title)+'</button>').join('')}
...
<section class="section"><div class="section-head"><h2>Hands-on labs mapped to lecture topics</h2><div class="section-note">6 circuit builds · 5 assembly traces</div></div><div class="topics">\${GUIDED_LABS.map(lab=>\`<article class="topic"><h3>\${esc(lab.lectureLabel)} · \${esc(lab.title)}</h3><p>\${lab.kind==='circuit'?'Create a fresh Digital circuit and build it through evidence checkpoints.':'Open original example code in the Instruction Trace Tutor and inspect visible model state.'}</p><button class="topic-button" data-guided-lab="\${lab.id}">\${lab.kind==='circuit'?'Start circuit build':'Start assembly trace'}</button></article>\`).join('')}</div></section>
