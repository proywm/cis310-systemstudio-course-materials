...
\${next?\`<section class="hero"><div class="eyebrow">Before class · next preparation</div><h2>\${esc(next.lectureLabel)}: \${esc(next.title)}</h2><p>\${esc(next.focus)}</p><p class="fine">Read → watch → try three questions. Each question opens the exact supporting source after you answer. Completion is self-reported and stays on this device.</p><div class="prep-steps"><div class="prep-step"><strong>1 · Read the open book</strong>\${next.readings.map((reading,index)=>\`<p>\${esc(reading.title)} · \${esc(reading.focus)}</p><button class="quiet" data-prep-open="reading" data-resource="\${next.resourceId}" data-index="\${index}">Open reading \${next.readings.length>1?index+1:''}</button>\`).join('')}<div><button class="completion \${next.read?'done':''}" data-prep-toggle="read" data-resource="\${next.resourceId}">\${next.read?'✓ Reading completed':'□ Mark all readings completed'}</button></div></div><div class="prep-step"><strong>2 · Watch the author</strong>\${next.authorVideos.map((video,index)=>\`<p>\${esc(video.title)} · \${esc(video.focus)}</p><button class="quiet" data-prep-open="video" data-resource="\${next.resourceId}" data-index="\${index}">Open video \${next.authorVideos.length>1?index+1:''}</button>\`).join('')}<div><button class="completion \${next.watched?'done':''}" data-prep-toggle="watched" data-resource="\${next.resourceId}">\${next.watched?'✓ Videos completed':'□ Mark all videos completed'}</button></div></div><div class="prep-step"><strong>3 · Check readiness</strong><p>\${esc(next.readinessPrompt)}</p><button class="primary" data-prep-check data-resource="\${next.resourceId}">Try 3 questions</button><p>\${next.practiceAttempts} local attempt\${next.practiceAttempts===1?'':'s'} so far</p></div></div></section>\`:''}
