...
export class PracticePanel implements vscode.Disposable {
  ...
  private constructor(
    ...
    this.panel.webview.html = practiceHtml(
      this.panel.webview,
      this.store.getDashboard(),
      this.store.getLearningPath(),
      companion
    );
      ...
      try {
        switch (request.type) {
          ...
          case 'reflect':
            ...
            break;
          case 'open-resource':
            await vscode.commands.executeCommand('systemstudioCis310.openLessonText', request.resourceId);
            break;
  ...
  private async confirmReset(): Promise<void> {
    const decision = await vscode.window.showWarningMessage(
      'Reset all local CIS 310 reading/video checkmarks, guided-lab progress, practice history, saved questions, confidence checks, and review dates?',
      { modal: true },
...
function enrichResult(result: PracticeAnswerResult, materials: CourseMaterials): object {
  ...
  return {
    ...
    preparation: preparation ? {
      ...
      lectureSlides: result.question.sourceMap.lectureSlides ?? []
    } : undefined
  };
}
...
function practiceHtml(
  ...
  const guidedLabs = JSON.stringify(GUIDED_LABS.map((lab) => ({
    id: lab.id,
    kind: lab.kind,
    title: lab.title,
    resourceId: lab.resourceId,
    requiredForModule: lab.requiredForModule
  }))).replaceAll('<', '\\u003c');
...
.prep-steps{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;margin-top:16px}.prep-step{border:1px solid var(--vscode-panel-border);border-radius:9px;padding:13px;background:color-mix(in srgb,var(--vscode-editor-background) 72%,transparent)}.prep-step strong{display:block;margin-bottom:6px}.prep-step p{font-size:.82rem;color:var(--vscode-descriptionForeground);margin:7px 0;line-height:1.4}.prep-step .actions{margin-top:10px}.source-tag{display:inline-block;border:1px solid var(--vscode-panel-border);border-radius:999px;padding:1px 6px;margin-right:4px;color:var(--vscode-foreground);font-size:.72rem}.source-tag.core{border-color:var(--vscode-testing-iconPassed);color:var(--vscode-testing-iconPassed)}.source-item{padding:6px 0;border-top:1px solid var(--vscode-panel-border)}.source-item:first-of-type{border-top:0}.source-item button{margin-top:4px}.completion{border:0;background:transparent;color:var(--vscode-foreground);cursor:pointer;padding:5px 0}.completion.done{color:var(--vscode-testing-iconPassed);font-weight:650}.practice-next{border:1px solid var(--vscode-panel-border);border-radius:10px;padding:18px;background:var(--vscode-sideBar-background)}details.section{border:1px solid var(--vscode-panel-border);border-radius:9px;padding:0 14px 14px}details.section summary{cursor:pointer;font-weight:650;padding:14px 0}.prep-list{display:grid;gap:9px}.prep-row{border-top:1px solid var(--vscode-panel-border);padding:12px 0}.prep-row:first-child{border-top:0}.prep-row-head{display:flex;justify-content:space-between;gap:12px}.prep-row h3{font-size:.95rem;margin:0}.prep-row p{margin:5px 0;color:var(--vscode-descriptionForeground);line-height:1.4}.prep-status{white-space:nowrap;font-size:.78rem;color:var(--vscode-descriptionForeground)}.source-links{display:flex;gap:12px;flex-wrap:wrap}.source-links button{border:0;background:transparent;color:var(--vscode-textLink-foreground);cursor:pointer;padding:0}.home-tabs{display:flex;flex-wrap:wrap;gap:6px;margin:0 0 18px;padding:6px;border:1px solid var(--vscode-panel-border);border-radius:9px;background:var(--vscode-sideBar-background)}.home-tabs button{flex:1 1 140px;border:1px solid transparent;color:var(--vscode-foreground);background:transparent}.home-tabs button[aria-selected="true"]{color:var(--vscode-button-foreground);background:var(--vscode-button-background)}.home-tab-panel[hidden]{display:none!important}.help-widget{position:fixed;right:22px;bottom:20px;z-index:4;display:grid;justify-items:end;gap:10px}.help-card{width:min(350px,calc(100vw - 44px));border:1px solid var(--vscode-panel-border);border-radius:12px;padding:15px;background:var(--vscode-editorWidget-background);box-shadow:0 8px 28px var(--vscode-widget-shadow)}.help-card[hidden]{display:none}.help-card-head{display:flex;justify-content:space-between;gap:12px;align-items:start}.help-card h2{font-size:1.05rem;margin:0 0 6px}.help-card p{margin:0;color:var(--vscode-descriptionForeground);line-height:1.4}.help-card .actions{margin-top:12px}.help-close{border:0;background:transparent;color:var(--vscode-foreground);cursor:pointer;font-size:1.15rem}.help-bubble{width:76px;height:76px;border:2px solid var(--vscode-focusBorder);border-radius:50%;display:grid;place-items:center;overflow:hidden;background:radial-gradient(circle at 50% 35%,#ffcb70 0 23%,var(--vscode-button-background) 72%);box-shadow:0 6px 20px var(--vscode-widget-shadow);cursor:pointer;animation:helper-float 3.2s ease-in-out infinite}.help-bubble img{width:72px;height:72px;object-fit:contain;object-position:center bottom;animation:helper-wave 2.8s ease-in-out infinite}.help-bubble:focus-visible{outline:3px solid var(--vscode-focusBorder);outline-offset:3px}@keyframes helper-float{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}@keyframes helper-wave{0%,45%,100%{transform:rotate(0)}55%{transform:rotate(-3deg)}65%{transform:rotate(3deg)}75%{transform:rotate(-2deg)}}@media(max-width:700px){.prep-steps{grid-template-columns:1fr}.prep-row-head{display:block}.prep-status{margin-top:4px}}@media(prefers-reduced-motion:reduce){.help-bubble,.help-bubble img{animation:none!important}}
...
<section id="home" class="screen"></section><section id="session" class="screen" hidden></section><section id="summary" class="screen" hidden></section><div id="toast" class="notice-toast" role="status" aria-live="polite" hidden></div>
	<aside class="help-widget" aria-label="CIS 310 help"><section id="helpCard" class="help-card" aria-labelledby="helpTitle"><div class="help-card-head"><div><h2 id="helpTitle" tabindex="-1">Questions? Start here.</h2><p>Attempt first. Use the local FAQ or U-M Codex CLI for a hint and reasoning feedback—not an answer or graded deliverable—or send a focused question before class.</p></div><button id="helpClose" class="help-close" aria-label="Close help message">×</button></div><div class="actions"><button class="secondary" data-help="faq">FAQ chat</button><button class="secondary" data-help="ai-tutor">Open U-M Codex coach</button><button class="quiet" data-help="before-class">Ask before class</button></div></section><button id="helpBubble" class="help-bubble" aria-label="Open CIS 310 help" aria-expanded="true" aria-controls="helpCard"><img src="${companion}" alt=""></button></aside>
</main><script nonce="${nonce}">
const vscode=acquireVsCodeApi();const TOPICS=${topics},GUIDED_LABS=${guidedLabs},READINESS_TARGET=${MODULE_READINESS_QUESTION_TARGET},CONFIDENCE_TARGET=${MODULE_CONFIDENCE_QUESTION_TARGET};let dashboard=${initialDashboard};let learningPath=${initialLearningPath};let currentQuestion=null;let selectedIndex=null;let confidence=null;let usedHint=false;let startedAt=Date.now();let answered=false;let viewState=vscode.getState()||{};
...
function showScreen(target){[home,session,summary].forEach(item=>item.hidden=item!==target);const heading=target===home?target.querySelector('[data-home-panel]:not([hidden]) h2'):target.querySelector('h2');if(heading){heading.setAttribute('tabindex','-1');heading.focus()}else target.querySelector('button:not([tabindex="-1"])')?.focus()}
...
function prepOpen(resourceId,target,readingIndex=0){vscode.postMessage({type:'open-preparation',resourceId,target,readingIndex})}
	const helpCard=document.getElementById('helpCard'),helpBubble=document.getElementById('helpBubble');function setHelp(open,moveFocus=false){helpCard.hidden=!open;helpBubble.setAttribute('aria-expanded',String(open));viewState={...viewState,helpOpen:open};vscode.setState(viewState);if(moveFocus)(open?document.getElementById('helpTitle'):helpBubble).focus()}helpBubble.onclick=()=>setHelp(helpCard.hidden,true);document.getElementById('helpClose').onclick=()=>setHelp(false,true);document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!helpCard.hidden&&helpCard.contains(document.activeElement)){event.preventDefault();setHelp(false,true)}});document.querySelectorAll('[data-help]').forEach(button=>button.onclick=()=>vscode.postMessage({type:'open-help',destination:button.dataset.help}));setHelp(viewState.helpOpen!==false);
session.addEventListener('keydown',event=>{const choice=event.target.closest('.choice[role="radio"]');if(!choice||!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End'].includes(event.key))return;const choices=[...session.querySelectorAll('.choice[role="radio"]')],index=choices.indexOf(choice);if(index<0)return;event.preventDefault();const nextIndex=event.key==='Home'?0:event.key==='End'?choices.length-1:(index+(['ArrowRight','ArrowDown'].includes(event.key)?1:-1)+choices.length)%choices.length;choices[nextIndex].focus();choices[nextIndex].click()});
function prepStatus(module){const tried=Math.min(module.practiceQuestionsAttempted||0,CONFIDENCE_TARGET);if(module.complete)return 'Read · watched · '+tried+'/'+CONFIDENCE_TARGET+' questions · hands-on complete';const done=[module.read?'read':null,module.watched?'watched':null,tried?tried+'/'+CONFIDENCE_TARGET+' questions':null,module.handsOnTotalLabs?module.handsOnCompletedLabs+'/'+module.handsOnTotalLabs+' required labs':null].filter(Boolean);return done.length?done.join(' · ')+' done':'Not started'}
	function sourceRole(module,type,index){const indexes=type==='reading'?module.readinessSources.readingIndexes:module.readinessSources.videoIndexes;return indexes.includes(index)?'Core preparation source':'Additional reference'}
	function sourceTag(module,type,index){const role=sourceRole(module,type,index);return '<span class="source-tag '+(role==='Core preparation source'?'core':'')+'">'+role+'</span>'}
function labsFor(resourceId){return GUIDED_LABS.filter(lab=>lab.resourceId===resourceId)}
function labButtons(resourceId){return labsFor(resourceId).map(lab=>'<button class="secondary" data-guided-lab="'+esc(lab.id)+'">'+(lab.kind==='circuit'?'Build: ':'Trace: ')+esc(lab.title)+(lab.requiredForModule?' · required':' · optional')+'</button>').join('')}
function bindPreparation(scope){scope.querySelectorAll('[data-prep-open]').forEach(button=>button.onclick=()=>prepOpen(button.dataset.resource,button.dataset.prepOpen,Number(button.dataset.index||0)));scope.querySelectorAll('[data-prep-toggle]').forEach(button=>button.onclick=()=>vscode.postMessage({type:'toggle-preparation',resourceId:button.dataset.resource,field:button.dataset.prepToggle}));scope.querySelectorAll('[data-prep-check]').forEach(button=>button.onclick=()=>start('practice','recommended',undefined,CONFIDENCE_TARGET,button.dataset.resource));scope.querySelectorAll('[data-guided-lab]').forEach(button=>button.onclick=()=>vscode.postMessage({type:'open-guided-lab',labId:button.dataset.guidedLab}))}
function bindHomeTabs(){const tabs=[...home.querySelectorAll('[data-home-tab]')],panels=[...home.querySelectorAll('[data-home-panel]')];function activate(id,focus=false){if(!tabs.some(tab=>tab.dataset.homeTab===id))id='next';tabs.forEach(tab=>{const selected=tab.dataset.homeTab===id;tab.setAttribute('aria-selected',String(selected));tab.tabIndex=selected?0:-1;if(selected&&focus)tab.focus()});panels.forEach(panel=>panel.hidden=panel.dataset.homePanel!==id);viewState={...viewState,learningTab:id};vscode.setState(viewState)}tabs.forEach(tab=>tab.onclick=()=>activate(tab.dataset.homeTab));home.querySelector('.home-tabs').onkeydown=event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;const index=tabs.indexOf(document.activeElement);if(index<0)return;event.preventDefault();const nextIndex=event.key==='Home'?0:event.key==='End'?tabs.length-1:(index+(event.key==='ArrowRight'?1:-1)+tabs.length)%tabs.length;activate(tabs[nextIndex].dataset.homeTab,true)};activate(viewState.learningTab||'next')}
function bindModuleUnitTabs(){const root=home.querySelector('[data-home-panel="modules"]'),preparation=root?.querySelector('details.section'),labs=root?.querySelector(':scope > section.section');if(!root||!preparation||!labs)return;const prepRows=[...preparation.querySelectorAll('.prep-row')],labCards=[...labs.querySelectorAll('.topic')],marker=document.createElement('div');preparation.before(marker);const bands=[{key:'foundations',label:'Logic foundations · 1–5',min:1,max:5},{key:'memory',label:'Memory & systems · 6–9',min:6,max:9},{key:'processor',label:'Processor & assembly · 10–13',min:10,max:13}],tabs=document.createElement('div'),panels=document.createElement('div');tabs.className='home-tabs module-unit-tabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','Module and lab groups');bands.forEach((band,index)=>{const tab=document.createElement('button'),panel=document.createElement('section'),prepTitle=document.createElement('h2'),prepList=document.createElement('div'),labTitle=document.createElement('h2'),topicList=document.createElement('div');tab.type='button';tab.id='module-unit-tab-'+band.key;tab.dataset.moduleUnitTab=band.key;tab.setAttribute('role','tab');tab.setAttribute('aria-controls','module-unit-panel-'+band.key);tab.setAttribute('aria-selected',String(index===0));tab.tabIndex=index===0?0:-1;tab.textContent=band.label;panel.id='module-unit-panel-'+band.key;panel.className='home-tab-panel';panel.dataset.moduleUnitPanel=band.key;panel.setAttribute('role','tabpanel');panel.setAttribute('aria-labelledby',tab.id);panel.hidden=index!==0;prepTitle.textContent='Module preparation';prepList.className='prep-list';prepRows.forEach((row,rowIndex)=>{const number=rowIndex+1;if(number>=band.min&&number<=band.max)prepList.append(row)});labTitle.textContent='Mapped hands-on labs';topicList.className='topics';labCards.forEach((card,cardIndex)=>{const number=Number(GUIDED_LABS[cardIndex]?.resourceId.split('-')[1]);if(number>=band.min&&number<=band.max)topicList.append(card)});panel.append(prepTitle,prepList,labTitle,topicList);tabs.append(tab);panels.append(panel)});preparation.remove();labs.remove();marker.replaceWith(tabs,panels);const buttons=[...tabs.querySelectorAll('[data-module-unit-tab]')];function activate(id,focus=false){if(!buttons.some(button=>button.dataset.moduleUnitTab===id))id='foundations';buttons.forEach(button=>{const selected=button.dataset.moduleUnitTab===id;button.setAttribute('aria-selected',String(selected));button.tabIndex=selected?0:-1;if(selected&&focus)button.focus()});panels.querySelectorAll('[data-module-unit-panel]').forEach(panel=>panel.hidden=panel.dataset.moduleUnitPanel!==id);viewState={...viewState,moduleUnitTab:id};vscode.setState(viewState)}buttons.forEach(button=>button.onclick=()=>activate(button.dataset.moduleUnitTab));tabs.onkeydown=event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;const index=buttons.indexOf(document.activeElement);if(index<0)return;event.preventDefault();const nextIndex=event.key==='Home'?0:event.key==='End'?buttons.length-1:(index+(event.key==='ArrowRight'?1:-1)+buttons.length)%buttons.length;activate(buttons[nextIndex].dataset.moduleUnitTab,true)};activate(viewState.moduleUnitTab||'foundations')}
...
<div class="home-tabs" role="tablist" aria-label="Learning dashboard sections"><button id="home-tab-next" role="tab" aria-selected="true" aria-controls="home-panel-next" data-home-tab="next">Next up</button><button id="home-tab-practice" role="tab" aria-selected="false" aria-controls="home-panel-practice" data-home-tab="practice" tabindex="-1">Practice &amp; review</button><button id="home-tab-modules" role="tab" aria-selected="false" aria-controls="home-panel-modules" data-home-tab="modules" tabindex="-1">Modules &amp; labs</button><button id="home-tab-progress" role="tab" aria-selected="false" aria-controls="home-panel-progress" data-home-tab="progress" tabindex="-1">Progress &amp; sources</button></div>
<section id="home-panel-next" class="home-tab-panel" role="tabpanel" aria-labelledby="home-tab-next" data-home-panel="next">
\${next?\`<section class="section practice-next"><div class="section-head"><h2>Start with the accessible HTML lecture</h2><div class="section-note">Primary lecture format</div></div><p>Open \${esc(next.lectureLabel)} for objectives, definitions, concise explanations, examples, self-checks, and tutor prompts before continuing to its reading, video, questions, and hands-on work.</p><div class="actions"><button class="primary" data-prep-open="lecture" data-resource="\${next.resourceId}">Open accessible HTML lecture</button></div></section>\`:''}
	\${next?\`<section class="hero"><div class="eyebrow">Before class · next preparation</div><h2>\${esc(next.lectureLabel)}: \${esc(next.title)}</h2><p>\${esc(next.focus)}</p><p class="fine">Self-paced: begin with the accessible HTML lecture, then use the mapped reading, transcript-checked videos, questions, and hands-on work. Five distinct question attempts complete the preparation checkpoint; continue through all eight for broader practice. This attempt threshold is not a readiness or mastery claim. Every answer includes an explanation, justification, and source trail. Progress stays on this device.</p><div class="prep-steps"><div class="prep-step"><strong>1 · Read before class</strong>\${next.readings.map((reading,index)=>\`<div class="source-item"><p>\${sourceTag(next,'reading',index)} \${esc(reading.title)} · \${esc(reading.focus)}</p><button class="quiet" data-prep-open="reading" data-resource="\${next.resourceId}" data-index="\${index}">Open this reading</button></div>\`).join('')}<div><button class="completion \${next.read?'done':''}" data-prep-toggle="read" data-resource="\${next.resourceId}">\${next.read?'✓ Reading step completed':'□ Mark reading step completed'}</button></div></div><div class="prep-step"><strong>2 · Watch before class</strong>\${next.authorVideos.map((video,index)=>\`<div class="source-item"><p>\${sourceTag(next,'video',index)} \${esc(video.title)} · \${esc(video.focus)}</p><button class="quiet" data-prep-open="video" data-resource="\${next.resourceId}" data-index="\${index}">Open this video</button></div>\`).join('')}<div><button class="completion \${next.watched?'done':''}" data-prep-toggle="watched" data-resource="\${next.resourceId}">\${next.watched?'✓ Video step completed':'□ Mark video step completed'}</button></div></div><div class="prep-step"><strong>3 · Check and build</strong><p>\${esc(next.readinessPrompt)}</p><button class="primary" data-prep-check data-resource="\${next.resourceId}">Practice all 8 questions</button><p><strong>\${Math.min(next.practiceQuestionsAttempted||0,CONFIDENCE_TARGET)} of \${CONFIDENCE_TARGET} distinct questions tried.</strong> Preparation checkpoint after \${READINESS_TARGET} attempts; full question set after \${CONFIDENCE_TARGET}. \${next.practiceAttempts} total local attempt\${next.practiceAttempts===1?'':'s'}.</p>\${labsFor(next.resourceId).length?\`<div class="actions">\${labButtons(next.resourceId)}</div><p><strong>Hands-on:</strong> \${next.handsOnCompletedLabs}/\${next.handsOnTotalLabs} required activities complete.</p>\`:''}</div></div></section>\`:\`<section class="hero"><div class="eyebrow">Preparation path complete</div><h2>All 13 modules include reading, video, questions, and required hands-on work</h2><p>Choose spaced review, saved questions, or any module below. The dashboard will not send you back to Lecture 1 unless you select it.</p></section>\`}
\${next&&labsFor(next.resourceId).length?\`<section class="section practice-next"><div class="section-head"><h2>Build or debug this lecture</h2><div class="section-note">Guided · formative · self-paced</div></div><p>Use the same mapped reading and video, predict before running, and collect visible circuit or actual machine-state evidence.</p><div class="actions">\${labButtons(next.resourceId)}</div></section>\`:''}
</section><section id="home-panel-practice" class="home-tab-panel" role="tabpanel" aria-labelledby="home-tab-practice" data-home-panel="practice" hidden>
...
</section><section id="home-panel-modules" class="home-tab-panel" role="tabpanel" aria-labelledby="home-tab-modules" data-home-panel="modules" hidden>
<details class="section" open><summary>All module preparation · \${completed}/\${learningPath.length} complete</summary><div class="prep-list">\${learningPath.map(module=>\`<article class="prep-row"><div class="prep-row-head"><h3>\${esc(module.lectureLabel)} · \${esc(module.title)}</h3><span class="prep-status">\${esc(prepStatus(module))}</span></div><p>\${esc(module.focus)}</p><div class="actions">\${module.readings.map((reading,index)=>\`<button class="quiet" data-prep-open="reading" data-resource="\${module.resourceId}" data-index="\${index}">Read · \${esc(sourceRole(module,'reading',index))} · \${esc(reading.focus)}</button>\`).join('')}\${module.authorVideos.map((video,index)=>\`<button class="quiet" data-prep-open="video" data-resource="\${module.resourceId}" data-index="\${index}">Watch · \${esc(sourceRole(module,'video',index))} · \${esc(video.focus)}</button>\`).join('')}<button class="quiet" data-prep-open="lecture" data-resource="\${module.resourceId}">Open accessible HTML lecture</button><button class="topic-button" data-prep-check data-resource="\${module.resourceId}">Practice 8 questions (\${Math.min(module.practiceQuestionsAttempted||0,CONFIDENCE_TARGET)}/\${CONFIDENCE_TARGET} tried)</button>\${labButtons(module.resourceId)}</div><div class="actions"><button class="completion \${module.read?'done':''}" data-prep-toggle="read" data-resource="\${module.resourceId}">\${module.read?'✓ Reading step':'□ Reading step'}</button><button class="completion \${module.watched?'done':''}" data-prep-toggle="watched" data-resource="\${module.resourceId}">\${module.watched?'✓ Video step':'□ Video step'}</button></div></article>\`).join('')}</div></details>
<section class="section"><div class="section-head"><h2>Hands-on labs mapped to module concepts</h2><div class="section-note">7 circuit builds · 7 actual NASM labs</div></div><div class="topics">\${GUIDED_LABS.map(lab=>\`<article class="topic"><h3>\${esc(lab.lectureLabel)} · \${esc(lab.title)}</h3><p>\${lab.kind==='circuit'?'Create a fresh Digital circuit and build it through evidence checkpoints.':'Open self-checking NASM source in the actual GDB workbench and inspect machine-state evidence.'} \${lab.requiredForModule?'Required for this module.':'Optional extension.'}</p><button class="topic-button" data-guided-lab="\${lab.id}">\${lab.kind==='circuit'?'Start circuit build':'Start NASM lab'}</button></article>\`).join('')}</div></section>
</section><section id="home-panel-progress" class="home-tab-panel" role="tabpanel" aria-labelledby="home-tab-progress" data-home-panel="progress" hidden>
