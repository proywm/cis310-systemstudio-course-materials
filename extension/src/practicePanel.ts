...
function enrichResult(result: PracticeAnswerResult, materials: CourseMaterials): object {
  ...
  return {
    ...
    preparation: preparation ? {
      readingLabel: preparation.readings.length === 1 ? preparation.readings[0]?.title : `${preparation.readings.length} mapped book readings`,
      videoTitle: preparation.authorVideo.title
    } : undefined
  };
}
...
function showScreen(target){[home,session,summary].forEach(item=>item.hidden=item!==target);target.querySelector('h2,button')?.focus()}
...
function prepStatus(module){if(module.complete)return 'Read · watched · checked';const done=[module.read?'read':null,module.watched?'watched':null,module.practiceAttempts?'checked':null].filter(Boolean);return done.length?done.join(' · ')+' done':'Not started'}
...
function renderHome(data,path=learningPath){dashboard=data;learningPath=path||learningPath;const dueLabel=data.due?data.due+' due':'Nothing due';const next=learningPath.find(module=>!module.complete)||learningPath[0];const completed=learningPath.filter(module=>module.complete).length;home.innerHTML=\`
\${next?\`<section class="hero"><div class="eyebrow">Before class · next preparation</div><h2>\${esc(next.lectureLabel)}: \${esc(next.title)}</h2><p>\${esc(next.focus)}</p><p class="fine">Read → watch → try three questions. Completion is self-reported and stays on this device.</p><div class="prep-steps"><div class="prep-step"><strong>1 · Read the open book</strong>\${next.readings.map((reading,index)=>\`<p>\${esc(reading.title)} · \${esc(reading.focus)}</p><button class="quiet" data-prep-open="reading" data-resource="\${next.resourceId}" data-index="\${index}">Open reading \${next.readings.length>1?index+1:''}</button>\`).join('')}<div><button class="completion \${next.read?'done':''}" data-prep-toggle="read" data-resource="\${next.resourceId}">\${next.read?'✓ Reading completed':'□ Mark reading completed'}</button></div></div><div class="prep-step"><strong>2 · Watch the author</strong><p>\${esc(next.authorVideo.title)}</p><button class="quiet" data-prep-open="video" data-resource="\${next.resourceId}">Open author video</button><div><button class="completion \${next.watched?'done':''}" data-prep-toggle="watched" data-resource="\${next.resourceId}">\${next.watched?'✓ Video completed':'□ Mark video completed'}</button></div></div><div class="prep-step"><strong>3 · Check readiness</strong><p>\${esc(next.readinessPrompt)}</p><button class="primary" data-prep-check data-resource="\${next.resourceId}">Try 3 questions</button><p>\${next.practiceAttempts} local attempt\${next.practiceAttempts===1?'':'s'} so far</p></div></div></section>\`:''}
