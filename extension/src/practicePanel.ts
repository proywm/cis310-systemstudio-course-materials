...
\${next&&labsFor(next.resourceId).length?\`<section class="section practice-next"><div class="section-head"><h2>Build or trace this lecture</h2><div class="section-note">Guided · formative · self-paced</div></div><p>Use the same mapped reading and video, predict before running, and collect visible circuit or machine-state evidence.</p><div class="actions">\${labButtons(next.resourceId)}</div></section>\`:''}
...
<section class="section"><div class="section-head"><h2>Hands-on labs mapped to module concepts</h2><div class="section-note">7 circuit builds · 8 assembly traces</div></div><div class="topics">\${GUIDED_LABS.map(lab=>\`<article class="topic"><h3>\${esc(lab.lectureLabel)} · \${esc(lab.title)}</h3><p>\${lab.kind==='circuit'?'Create a fresh Digital circuit and build it through evidence checkpoints.':'Open original example code in the Instruction Trace Tutor and inspect visible model state.'} \${lab.requiredForModule?'Required for this module.':'Optional extension.'}</p><button class="topic-button" data-guided-lab="\${lab.id}">\${lab.kind==='circuit'?'Start circuit build':'Start assembly trace'}</button></article>\`).join('')}</div></section>
