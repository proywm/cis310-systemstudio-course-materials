...
export async function exportFall2026CourseCalendar(): Promise<void> {
  ...
  const action = await vscode.window.showInformationMessage(
    `Exported 27 confirmed Monday/Wednesday class meetings, ${FALL_2026_CLASS_TIME_LABEL}, ${FALL_2026_CLASS_LOCATION}, plus official term milestones and the CIS 310 final-project examination-week window. Assignment deadlines and exact final-project logistics are not inferred.`,
    'Open File'
  );
  if (action === 'Open File') {
...
<style>
  ...
  <div class="schedule-details"><section class="schedule-detail"><h2>Class meetings</h2><p>Mondays and Wednesdays, <strong>${FALL_2026_CLASS_TIME_LABEL}</strong><br><strong>${FALL_2026_CLASS_LOCATION}</strong></p></section><section class="schedule-detail"><h2>Instructor</h2><p><strong>${CIS310_INSTRUCTOR.name}</strong>, ${CIS310_INSTRUCTOR.title}<br>${CIS310_INSTRUCTOR.email} · ${CIS310_INSTRUCTOR.phone}<br>${FALL_2026_OFFICE_LOCATION}</p></section><section class="schedule-detail"><h2>Graduate Student Instructor</h2><p><strong>${CIS310_GSI.label}</strong><br>${CIS310_GSI.detail}</p></section><section class="schedule-detail"><h2>Instructor office hours</h2><p>${FALL_2026_OFFICE_HOURS_LABEL}<br><strong>${FALL_2026_OFFICE_LOCATION}</strong></p></section></div>
  <div class="notice"><strong>Starts Wednesday, August 26.</strong> The department's August 14, 2026 schedule confirms the class time and room. The culminating presentation demonstrates the cumulative 4-bit processor built through Implementations 1–3 and its released assembly program during final examination week; its exact date, time, room, order, and submission deadline are <strong>to be announced in Canvas</strong>.</div>
  <div class="actions"><button data-action="open-canvas">Open Canvas</button><button data-action="open-syllabus">Open accessible syllabus</button><button class="secondary" data-action="export-calendar">Export .ics</button><button class="secondary" data-action="open-official-calendar">Official academic calendar</button></div></header>
  ${monthSections}
