...
4. `PracticeQuestion.sourceMap` records the exact reading and video indexes for every question. `PreClassModule.readinessSources` does the same for each readiness prompt.
...
| Lecture | Readiness and practice coverage | Exact assigned reading | Exact assigned video |
|---|---|---|---|
...
| 8 | Status/configuration/data registers; polling cost; status-before-data | Ch. 15 §§15.9.1–15.9.2 | Ep 087, with Ep 086 for I/O context |
...
The extension shows all assigned sources during preparation. After a practice response, it shows only the reading(s) and video(s) mapped to that particular question.
