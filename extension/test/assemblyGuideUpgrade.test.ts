...
describe('pre-0.20 assembly-workspace migration', () => {
  it('archives all stale guides, installs current guides, and preserves student source', async () => {
    ...
    try {
      ...
      assert.match(await readFile(path.join(target, 'README.md'), 'utf8'), /systemstudio-assembly-guide: 0\.20/);
      assert.match(await readFile(path.join(target, 'COMPATIBILITY.md'), 'utf8'), /systemstudio-assembly-compatibility: 0\.20/);
      assert.match(await readFile(path.join(target, 'OPEN_BOOK.md'), 'utf8'), /systemstudio-open-assembly-book: 0\.20/);
      assert.match(await readFile(path.join(target, 'README-pre-0.20.md'), 'utf8'), /Embedded Assembly Lab/);
      assert.match(await readFile(path.join(target, 'COMPATIBILITY-pre-0.20.md'), 'utf8'), /Portable embedded subset/);
      assert.match(await readFile(path.join(target, 'LEGACY-IRVINE32_PROFILE.md'), 'utf8'), /Classroom profile/);
