...
Before each topic, use SystemStudio's short sequence \textbf{read $\rightarrow$ watch $\rightarrow$ retrieve}: read the mapped sections of the open Tarnoff text, watch the targeted official author videos, and try three readiness questions without reopening the sources. Bring one unresolved point to class. Class meetings then combine explanation, prediction, worked examples, circuit or assembly demonstrations, and short evidence checks. For project work, use \textbf{predict $\rightarrow$ build $\rightarrow$ test $\rightarrow$ inspect evidence $\rightarrow$ explain}.
