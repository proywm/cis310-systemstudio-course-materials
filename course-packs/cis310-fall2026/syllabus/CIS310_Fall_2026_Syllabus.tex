...
Before each topic, use SystemStudio's self-paced sequence \textbf{accessible HTML lecture $\rightarrow$ read $\rightarrow$ watch $\rightarrow$ retrieve $\rightarrow$ build or debug}: begin with the primary structured HTML lecture, continue with focused open-text sections and transcript-checked author videos, and try five distinct questions without reopening the sources. Continue through all eight questions for broader practice. These attempt counts do not certify readiness or mastery. The untagged lecture PDF is an optional visual archive, not the primary lecture format. When a required hands-on lab is mapped, complete \textbf{predict $\rightarrow$ build or step $\rightarrow$ inspect evidence $\rightarrow$ explain}. Formative circuit labs create fresh files and open them in the complete upstream Digital application. Assembly labs invoke actual NASM, GNU linking, executable tests, and an actual GDB session; the separately labeled trace tutor remains optional conceptual practice and is not build evidence. Neither supplies a graded assignment artifact. Bring one unresolved point to class. Class meetings then combine explanation, prediction, worked examples, circuit or assembly demonstrations, and short evidence checks. For project work, use \textbf{predict $\rightarrow$ build $\rightarrow$ test $\rightarrow$ inspect evidence $\rightarrow$ explain}.
...
SystemStudio CIS 310 & The VS Code extension maps all 13 lecture topics to primary accessible HTML lectures, focused open-book readings, transcript-checked author videos, full eight-question practice sets, and hands-on work where appropriate. It also provides Full Digital, seven guided circuit builds, seven actual NASM/GDB labs (including search algorithms), quiz mode, confidence checks, explanation and justification, review, the syllabus, calendar, 13 optional offline visual PDF archives, seven coursework/final-project references, Assignment Mission Control, setup checks, a guided tutorial, and a local help router.\\
...
Actual NASM Workbench & Fall 2026 student-authored x86 work uses NASM 32-bit. SystemStudio invokes actual NASM, GNU \texttt{ld}, an ELF32 executable, and GDB. It uses complete host tools on x86 Linux or a locally built course container with NASM, GNU \texttt{ld}, GDB, and QEMU-i386 on Windows/macOS or as a fallback. The workbench exposes source location, Intel disassembly, registers, EFLAGS, stack, memory, breakpoints, stepping, output, and exit status.\\
Instruction Trace Tutor & A separately labeled, bounded source-level visualization remains available for optional prediction practice. It is not NASM or GDB, emits no machine code, and cannot establish that source assembled successfully.\\
Student computer & A current desktop version of VS Code and enough access to install the course extension. Windows/macOS students need Docker Desktop for the portable Digital and NASM course environments; SystemStudio does not install Docker silently. Contact the instructor promptly if hardware, Java, Docker, accessibility, or installation creates a barrier.\\
\end{tabularx}
...
\begin{itemize}
  ...
  \item \textbf{Project 1:} 4-bit registers/data memory and 8-bit instruction storage;
...
\textbf{Required open assembly reference:} Paul A. Carter, \emph{PC Assembly Language}, available without charge from the \href{https://pacman128.github.io/pcasm/}{author's official page}; all examples use NASM 32-bit. Because the source PDF is untagged, the structured SystemStudio HTML lesson is the primary accessible introduction and the book is assigned in focused sections. Use the \href{https://www.nasm.us/doc/}{official NASM manual} as the syntax reference.
...
\begin{itemize}
  \item \textbf{Participation quizzes:} Complete participation quizzes and in-class evidence checks during the class meeting in which they are assigned. Late participation work is not accepted. The two lowest quiz scores are dropped when the participation category is calculated.
  ...
  \item \textbf{Final cumulative 4-bit processor presentation and demonstration:} The final assessment presents the same 4-bit processor developed through the three implementation assignments, its 8-bit instructional-ISA program, test evidence, and a live presentation/demonstration during final examination week. It may be completed individually or in an instructor-authorized team of no more than two students. Every team member must participate in and explain the processor, program encoding, test evidence, and demonstration. Any separately assigned IA-32 NASM work is a different toolchain artifact and does not execute on the instructional processor. The exact date, time, room, required artifacts, and presentation format will be announced in Canvas. The final deliverable and demonstration are not accepted late except when the instructor approves an accommodation or documented extenuating circumstance.
...
\textbf{Final examination/project demonstration:} The cumulative 4-bit processor and assembly-program presentation/demonstration will take place during Fall 2026 final examination week. The \textbf{exact date, time, room, presentation order, and submission deadline are to be announced in Canvas}. Students should keep the examination period available until the official course announcement is posted.
...
\begin{itemize}
  ...
  \item The optional U-M Codex CLI learning coach runs in VS Code with each student's own U-M configuration. Use it for incremental hints, explanations, and formative debugging, then verify claims against the mapped course source. The extension neither contains an instructor credential nor reads or stores the student's U-M key.
  ...
  \item Seek help early when a prerequisite, setup step, circuit behavior, NASM build/debug result, assignment instruction, or team interaction is unclear.
\end{itemize}
...
Collaboration and generative-AI permissions are assignment-specific. If a Canvas assignment does not clearly authorize a type of assistance, ask the instructor before using it in submitted work. For an ungraded SystemStudio preparation, practice, or tutorial question, commit to an answer and one reason before requesting an AI hint or critique; asking for the answer first defeats the retrieval activity. The U-M Codex CLI learning coach may support formative learning through incremental hints, analogous examples, source links, and checks of student-supplied reasoning. For homework, projects, quizzes, exams, reports, and other graded work, it must not be treated as an answer key or used to produce a final answer, finished circuit, complete program, report prose, or other submission-ready artifact. AI output can be inaccurate; students remain responsible for checking it against course evidence and for the work they submit. SystemStudio supplies course instructions and a reviewed starter prompt but cannot guarantee model compliance. The workflow may organize instructor-approved materials and explain visible tool evidence, but it does not replace the student's reasoning or certify that an answer is correct.
...
26 & Wed Dec 2 & Actual NASM/GDB workbench: registers, flags, stack, memory, and disassembly; Homework 3 / Project 3 preparation checkpoint\\
