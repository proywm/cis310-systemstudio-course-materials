...
6. Use the **Actual NASM Debug Workbench** for real execution: NASM → GNU ELF32 link → self-checking executable → actual GDB evidence. The extension uses host tools on x86 Linux or the portable course container on Windows/macOS. Record the build result, output/exit status, breakpoint label, and relevant register/flag/stack/memory/disassembly evidence.
