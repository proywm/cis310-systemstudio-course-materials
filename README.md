...
| Current status | CIS 310 extension v0.8.0 combines a Fall 2026 syllabus and course calendar, offline course PDFs, blank/assignment circuit creation, separated homework and projects, and no-setup Irvine32 Classroom/NASM IA-32 teaching profiles; classroom validation remains pending |
...
The accelerated Fall 2026 extension now manages a checksum-verified Digital v0.31 installation, detects Java, creates valid blank `.dig` files, opens circuits in Digital when a graphical desktop is available, renders Digital-generated SVG previews inside VS Code, runs embedded circuit tests, publishes eligible circuits to Test Explorer, and generates a starter workspace. The Course Materials view provides a packaged Fall 2026 syllabus PDF, 13 embedded offline PDF presentations, three homework references, three project references, assignment-specific circuit buttons, and lecture mappings. A visual calendar covers all 27 Monday/Wednesday meetings beginning August 26 and can export `.ics` events without guessing the class time. The embedded assembly lab supports Irvine32 Classroom and NASM IA-32 teaching profiles without Docker, Visual Studio, or a host assembler. The logically grouped sidebar, skippable/replayable tutorial, exact Fall 2026 Canvas link, and local evidence-and-routing helper connect these workflows. Syllabus and presentation viewing require no Google Drive access; Canvas controls current requirements and submission. This remains a technical MVP, not evidence of learning impact or classroom readiness. The generative AI coach, custom processor canvas, help-packet workflow, and broader CIS 450 environment remain senior-design workstreams.
