...
| Current status | CIS 310 extension v0.5.0 combines offline course PDFs, blank/assignment circuit creation, separated homework and projects, and a no-setup embedded IA-32 MASM/NASM teaching lab; classroom validation remains pending |
...
The accelerated extension now manages a checksum-verified Digital v0.31 installation, detects Java, creates valid blank `.dig` files, opens circuits in Digital when a graphical desktop is available, renders Digital-generated SVG previews inside VS Code, runs embedded circuit tests through the official CLI, publishes eligible circuits to Test Explorer, and generates a starter workspace. The Course Materials view provides 13 bundled offline PDF presentations, separately grouped homework and project references, assignment-specific circuit-creation buttons, and explicit lecture mappings. Version 0.5.0 adds an original, extension-native IA-32 teaching interpreter with a documented MASM/NASM classroom subset and visible registers, flags, memory, stack, output, and trace; it needs no Docker or host assembler and does not claim production toolchain compatibility. The source pack is from Fall 2025 and remains gated for instructor review; it does not replace current Canvas instructions. This is a technical MVP, not yet evidence of learning impact or classroom readiness. The AI coach, guided-practice system, custom processor canvas, help-packet workflow, and broader CIS 450 environment remain senior-design workstreams.
